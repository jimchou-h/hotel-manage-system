<template>
  <div style="height: 100%">
    <div class="header-wrapper" v-if="$store.state.personal">
      <span class="logo-font">青年酒店</span>
      <el-button class="close-button" type="info" title="收起" @click="handleClose()"><i class="iconfont">&#xe985;</i></el-button>
      <span class="welcome-tips">
        <span v-if="!name">尊敬的{{ positions[position] }}，您好&nbsp;</span>
        <div class="person boxHide" @click="handleShowBox($event)" v-if="name">
          <i class="iconfont boxHide" title="个人信息">&#xe610;</i>
          <div class="showPerSonBox boxHide" v-if="isShow">
            <div class="show-name boxHide">姓名: {{ name }}</div>
            <div class="show-position boxHide">职位: {{ positions[position] }}</div>
            <div class="show-button boxHide" v-if="!card" @click="handleCard()">立刻打卡</div>
            <div class="show-button boxHide" style="background: gray" v-else>已打卡</div>
          </div>
        </div>
        <span class="logout" @click="handleLogout()"><i class="iconfont" style="color: red" title="退出">&#xe600;</i></span>
      </span>
    </div>
    <div class="section" style="display: flex; width: 100%">
      <div class="menu-wrapper">
        <el-menu v-if="navs.length > 0" class="el-menu-vertical-demo" :collapse="isClose" background-color="#000"
          :router="true" text-color="#fff" active-text-color="#ffd04b" :default-active="$route.path">
  ...
  export default {
    ...
    methods: {
      handleCard() {
        ...
        let cardTime = this.$Methods.getStandardTime(new Date())
      ...
      handleShowBox(e) {
        if (e.target.className.indexOf('person') > -1 || e.target.className.indexOf('iconfont') > -1) {
      ...
      handleClick(e) {
        if (e.target.className.lastIndexOf('boxHide') === -1) {
          this.isShow = false
        }
      },
      __dataInit() {
