<template>
  <div class="wrapper">
    <div class="cleaner-add">
      <el-button type="primary" @click="handleSetRoomClean()">设置为已清理</el-button>
      <el-button type="primary" @click="handleSetRoomUnclean()">设置为未清理</el-button>
      <div class="search-wrapper">
        <el-select v-model="cleanerQuery.q" placeholder="清理状态筛选">
          <el-option label="全部" value>
          </el-option>
          <el-option label="已清理" value="1">
          </el-option>
          <el-option label="未清理" value="0">
          </el-option>
        </el-select>
      </div>
    </div>
    <div class="table-wrapper">
      <ele-table ref="table" :tableData="cleanerTableData" :tableTitle="cleanerTableTitle" :needList="needList"></ele-table>
    </div>
  ...
  export default {
    ...
    methods: {
      handleSetRoomUnclean() {
        ...
        if (!ids.length) {
          this.$message.error('选中的客房状态和即将要修改的客房状态重复');
          return;
        }
        this.$Service.setRoomUnclean({
          ids: ids
        }).then(res => {
          if (res.code == 1) {
            this.$message({
