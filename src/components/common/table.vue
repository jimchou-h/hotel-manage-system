...
<template>
  <div>
    <el-table ref="table" :data="tableData" border stripe style="width: 100%">
      <el-table-column v-if="needList && needList.selection" type="selection" align="center"></el-table-column>
      <template v-for="data of tableTitle">
        <el-table-column :prop="data.td" :label="data.th" :key="data.td"></el-table-column>
      </template>
      <el-table-column label="职位" v-if="needList && needList.position">
        <template slot-scope="scope">
          {{ positions[scope.row.position] }}
        </template>
      </el-table-column>
      <el-table-column label="是否已经预定" v-if="needList && needList.isBook">
        <template slot-scope="scope">
          {{ scope.row.isBook == 1 ? '是' : '否' }}
        </template>
      </el-table-column>
      <el-table-column label="是否可用" v-if="needList && needList.isRent">
        <template slot-scope="scope">
          {{ scope.row.isRent == 1 ? '是' : '否' }}
        </template>
      </el-table-column>
      <el-table-column label="是否已清理" v-if="needList && needList.isClean">
        <template slot-scope="scope">
          {{ scope.row.isClean == 1 ? '已清理' : '未清理' }}
        </template>
      </el-table-column>
      <el-table-column label="是否已入住" v-if="needList && needList.isLive">
        <template slot-scope="scope">
          {{ scope.row.isLive == 1 ? '是' : '否' }}
        </template>
      </el-table-column>
      <el-table-column label="是否已支付" v-if="needList && needList.isPay">
        <template slot-scope="scope">
          {{ scope.row.isPay == 1 ? '已支付' : '未支付' }}
        </template>
      </el-table-column>
      <el-table-column label="状态" v-if="needList && needList.situation">
        <template slot-scope="scope">
          {{ scope.row.situation != 0 ? (scope.row.situation == 1 ? '已完成' : '无法完成，已和顾客联系获得顾客理解') : '处理中' }}
        </template>
      </el-table-column>
      <el-table-column label="经理是否已同意" v-if="needList && needList.isAllow">
        <template slot-scope="scope">
          {{ scope.row.isAllow == 0 ? '审批中' : (scope.row.isAllow == 1 ? '已同意' : '不同意') }}
        </template>
      </el-table-column>
      <el-table-column label="是否已采购" v-if="needList && needList.isBuy">
        <template slot-scope="scope">
          {{ scope.row.isBuy == 1 ? '是' : '否' }}
        </template>
      </el-table-column>
      <el-table-column label="操作" v-if="operate" :width="getOperate()" fixed="right">
        <template slot-scope="scope">
          <div class="operate-wrapper">
            <img v-if="operate.edit" class="operate" src="@/assets/img/operate-edit.png" srcset="@/assets/img/operate-edit@2x.png 2x"
              @click="handleEditClick(scope.row)" />
            <img v-if="operate.delete && scope.row.position != 'admin'" @click="handleOperateDelete(scope.row)" class="operate"
              src="@/assets/img/operate-delete.png" srcset="@/assets/img/operate-delete@2x.png 2x" />
            <div class="operate" title="预定" style="background: #57ABD8; display: inline-block" v-if="scope.row.isClean && !scope.row.livetime && !scope.row.isBook && operate.book" @click="handleBookClick(scope.row)">
              <i class="iconfont watch">&#xe6ae;</i>
            </div>
            <div class="operate" title="取消预定" style="background: red; display: inline-block" v-if="!scope.row.livetime && scope.row.isBook && operate.book" @click="handleUnbookClick(scope.row)">
              <i class="iconfont unbook">&#xe604;</i>
            </div>
            <div class="operate" title="入住" style="background: #57ABD8; display: inline-block" v-if="scope.row.isClean && !scope.row.livetime && operate.live" @click="handleSetLive(scope.row)">
              <i class="iconfont watch">&#xe67e;</i>
            </div>
            <div class="operate" title="退房" style="background: red; display: inline-block" v-if="scope.row.isClean && scope.row.livetime && operate.live" @click="handleSetLeave(scope.row)">
              <i class="iconfont watch">&#xe6ee;</i>
            </div>
            <!--
            <div
